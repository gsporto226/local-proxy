...
describe("e2e: statusline renders the session's recorded stats", () => {
  ...
  beforeAll(async () => {
    ...
    // Providers get keys only from auth.json (see mock.test.ts); seed it in the
    // isolated config dir so both mock providers are "connected" for routing.
    writeFileSync(
      join(cfgDir, "auth.json"),
      JSON.stringify(
        { mock_openai: { type: "api", key: "test-key" }, mock_anthropic: { type: "api", key: "test-key" } },
        null,
        2,
      ),
    );
